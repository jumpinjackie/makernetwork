...
export default function mapReducer(state = INITIAL_STATE, action = { type: '', payload: null }) {
    switch (action.type) {
        case MAP_GET_ACTIVITIES_SINCE_PENDING:
            return state.merge(fromJS({
                hasError: false,
                isLoading: true
            }));
        case MAP_GET_ACTIVITIES_SINCE_SUCCESS:
            ...
            return newState;
        case MAP_GET_ACTIVITIES_SINCE_ERROR:
            return state.merge(fromJS({
                hasError: true,
                isLoading: false
            }));
